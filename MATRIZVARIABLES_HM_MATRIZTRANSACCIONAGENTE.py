...
# || NOMBRE     		  : MATRIZVARIABLES_HM_MATRIZTRANSACCIONAGENTE.py
...
# || 7   	  RUBEN QUISPE                  NTT DATA      RODRIGO ROJAS     19/09/25     Ajuste de proceso
# ************************************************************************************************************************************************
...
dbutils.widgets.text(name="PRM_TABLA_SEGUNDATRANSPUESTA_TMP", defaultValue='hm_matriztransaccionagente_tmp')
dbutils.widgets.text(name="PRM_TABLA_PARAMETROMATRIZVARIABLES", defaultValue='mm_parametromatrizvariables')
dbutils.widgets.text(name="PRM_FECHA_RUTINA", defaultValue='1990-01-01')
...
VAL_DESTINO_NAME = PRM_ESQUEMA_TABLA+"."+PRM_TABLA_SEGUNDATRANSPUESTA
...
def write_temp_table(df, tmp_table_name, ruta_salida):
    df.write \
        ...
        .saveAsTable(tmp_table_name, path=ruta_salida )
...
def extraccion_info_base_cliente(nCodMes: str,
    ...
    tmp_table_1 = f'{esquemaTabla}.{carpetaDetConceptoClienteSegundaTranspuesta}_tmp'.lower()
    ...
    (
        ...
        .partitionBy('CODMES')
        .mode(CONS_WRITE_TEXT_MODE_NAME)
        .saveAsTable(tmp_table_1,path=ruta_salida_1)
    )
...
def extraccionInformacion12Meses(codMes, codMes12Atras, carpetaDetConceptoClienteSegundaTranspuesta, carpetaClientePrimeraTranspuesta):
    ...
    dfClienteCuc = spark.table(f'{PRM_ESQUEMA_TABLA}.{carpetaDetConceptoClienteSegundaTranspuesta}_tmp') 
    ...
    tmp_table_2 = f'{PRM_ESQUEMA_TABLA}.{carpetaClientePrimeraTranspuesta}_tmp'.lower()
...
def main():
    ...
    tmp_table_1 = f'{PRM_ESQUEMA_TABLA}.{carpetaDetConceptoClienteSegundaTranspuesta}_tmp'.lower()
    tmp_table_2 = f'{PRM_ESQUEMA_TABLA}.{carpetaClientePrimeraTranspuesta}_tmp'.lower()
