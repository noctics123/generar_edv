...
# || NOMBRE     		  : HM_MATRIZTRANSACCIONPOSMACROGIRO_EDV_v2.py
# || TABLA DESTINO	      : BCP_EDV_MATRIZVARIABLES.HM_MATRIZTRANSACCIONPOSMACROGIRO
# || TABLAS FUENTES		  : BCP_EDV_MATRIZVARIABLES.HM_CONCEPTOTRANSACCIONPOS
...
# || 2.3      AUTOMATICO                  CLAUDE AI     RODRIGO ROJAS      23/09/2025   Conversión a versión EDV v2 con tablas managed.
# *************************************************************************************************************
...
dbutils.widgets.text(name="PRM_TABLE_NAME", defaultValue='HM_MATRIZTRANSACCIONPOSMACROGIRO_RUBEN')
...
dbutils.widgets.text(name="PRM_FECHA_RUTINA", defaultValue='2025-09-02')
dbutils.widgets.text(name="PRM_TABLA_SEGUNDATRANSPUESTA_TMP", defaultValue='hm_matriztransaccionposmacrogiro_tmp_ruben')
...
# Configuración de esquemas y rutas
...
def extraccionInformacion12Meses(codMes, codMes12Atras, carpetaDetConceptoClienteSegundaTranspuesta, carpetaClientePrimeraTranspuesta):
    ...
    dfInfo12Meses.write.format(funciones.CONS_FORMATO_DE_ESCRITURA_EN_DISCO).mode(funciones.CONS_MODO_DE_ESCRITURA).save(PRM_CARPETA_RAIZ_DE_PROYECTO+"/temp/"+carpetaClientePrimeraTranspuesta+"/CODMES="+str(codMes))

    #Leemos el resultado calculado
    dfInfo12Meses = spark.read.format(funciones.CONS_FORMATO_DE_ESCRITURA_EN_DISCO).load(PRM_CARPETA_RAIZ_DE_PROYECTO+"/temp/"+carpetaClientePrimeraTranspuesta+"/CODMES="+str(codMes))
...
def agruparInformacionMesAnalisis(dfInfo12Meses):
    ...
    for colName in colsToExpandMonto:
        original_cols.extend([func.round(col(colName + "_PRM_U6M")/col(colName + "_PRM_P6M"), 8).alias(colName + "_G6M"),
    ...
    #Se calcula el crecimiento 6,3 y 1 para las variables tipo TKT
    original_cols = dfMatrizVarTransaccionPosMacrogiroMont.columns 	
    ...
    #Se calcula el crecimiento 6,3 y 1 para las variables tipo Cantidad
    original_cols = dfMatrizVarTransaccionPosMacrogiroMont.columns 	
...
def main():
    ...
    for index,row in  totalMeses.iterrows():
        ...
        # Crear tabla temporal de cliente base directamente en esquema EDV
        df_cliente_cuc = spark.sql(f"""
            SELECT
                CODMES,
                CODUNICOCLI CODUNICOCLI_LAST,
                MAX(CODINTERNOCOMPUTACIONALCONCEPTO) CODINTERNOCOMPUTACIONAL_LAST
            FROM
                {PRM_ESQUEMA_TABLA}.HM_DETCONCEPTOCLIENTE
            WHERE
                CODMES = {codMes}
            GROUP BY CODMES, CODUNICOCLI
        """)

        # Guardar en disco como archivo temporal
        df_cliente_cuc.write.format(funciones.CONS_FORMATO_DE_ESCRITURA_EN_DISCO).mode(funciones.CONS_MODO_DE_ESCRITURA).save(PRM_CARPETA_RAIZ_DE_PROYECTO+"/temp/"+carpetaDetConceptoClienteSegundaTranspuesta+"/CODMES="+str(codMes))
        ...
        #Los archivos temporales en disco se manejan automáticamente por Spark
        print("Proceso completado. Archivos temporales en /temp/ pueden ser limpiados posteriormente si es necesario.")
